...
class PersonalizationReferenceCatalogTest(unittest.TestCase):
    def test_reference_catalog_matches_official_protocol_reset(self) -> None:
        ...
        self.assertEqual(
            {entry["id"] for entry in references if entry["metric_fidelity"] == "proxy_local"},
            {
                "alpsbench-extraction",
                "alpsbench-update",
                "alpsbench-retrieval",
        ...
        alpsbench_extraction = next(entry for entry in references if entry["id"] == "alpsbench-extraction")
        self.assertEqual(alpsbench_extraction["task_id"], "alpsbench-extraction")
        self.assertEqual(alpsbench_extraction["task_shape"], "agentic_open_ended")
        self.assertEqual(alpsbench_extraction["official_metric_backend"], "hybrid")
        self.assertEqual(alpsbench_extraction["metric_fidelity"], "proxy_local")

        alpsbench_update = next(entry for entry in references if entry["id"] == "alpsbench-update")
        self.assertEqual(alpsbench_update["task_id"], "alpsbench-update")
        self.assertEqual(alpsbench_update["task_shape"], "agentic_open_ended")
        self.assertEqual(alpsbench_update["official_metric_backend"], "hybrid")
        self.assertEqual(alpsbench_update["metric_fidelity"], "proxy_local")

        alpsbench_retrieval = next(entry for entry in references if entry["id"] == "alpsbench-retrieval")
        self.assertEqual(alpsbench_retrieval["task_id"], "alpsbench-retrieval")
        self.assertEqual(alpsbench_retrieval["task_shape"], "agentic_open_ended")
        self.assertEqual(alpsbench_retrieval["official_metric_backend"], "llm_judge")
        self.assertEqual(alpsbench_retrieval["metric_fidelity"], "proxy_local")

        alpsbench_utilization = next(entry for entry in references if entry["id"] == "alpsbench-utilization")
        self.assertEqual(alpsbench_utilization["task_id"], "alpsbench-utilization")
        self.assertEqual(alpsbench_utilization["task_shape"], "dialogue_judgement")
        self.assertEqual(alpsbench_utilization["official_metric_backend"], "llm_judge")
        self.assertEqual(alpsbench_utilization["metric_fidelity"], "proxy_local")
