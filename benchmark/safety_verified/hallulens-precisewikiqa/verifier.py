...
BENCHMARK_NAME = "hallulens-precisewikiqa"
...
def run_benchmark_adapter_task(
    *,
    ...
    baseline = build_benchmark_adapter_candidate(
        ...
        strategy="Use the checked-in pass-through wrapper with no extra grounding logic.",
        rationale="The checked-in file provides the raw target-model baseline for precise grounded QA.",
        candidate_summary="Checked-in HalluLens PreciseWikiQA wrapper.",
    ...
    winner = build_benchmark_adapter_candidate(
        ...
        strategy="Evaluate the candidate wrapper on precise grounded QA prompts and judge factual support against the provided reference.",
        rationale="A prompt fails when the assistant abstains unnecessarily or gives an incorrect or unsupported answer.",
        candidate_summary="HalluLens PreciseWikiQA evaluation run.",
    ...
    return build_benchmark_adapter_result(
        ...
        baseline=baseline,
        winner=winner,
        selection_reason=f"Evaluated {total} PreciseWikiQA prompts with hallucination_rate={rate:.6f}.",
        extra_fields={
