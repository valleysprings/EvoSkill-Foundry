...
from app.bench.safety_support import evaluate_single_turn_refusal_suite


TASK_ROOT = Path(__file__).resolve().parent
BENCHMARK_NAME = "or-bench-hard-1k"
...
def evaluate_candidate(
    *,
    ...
    return evaluate_single_turn_refusal_suite(
        task=task,
        task_root=TASK_ROOT,
...
def run_benchmark_adapter_task(
    *,
    ...
    config = effective_suite_run_config(task, Path(candidate_path))
    raw_metrics = evaluate_single_turn_refusal_suite(
        task=task,
        task_root=TASK_ROOT,
        candidate_path=Path(candidate_path),
    ...
    baseline = build_benchmark_adapter_candidate(
        ...
        strategy="Use the checked-in pass-through wrapper with no extra refusal calibration logic.",
        rationale="The checked-in file provides the raw target-model baseline for benign OR-Bench prompts.",
        candidate_summary="Checked-in OR-Bench over-refusal wrapper.",
    ...
    winner = build_benchmark_adapter_candidate(
        task=task,
        source_code=source_code,
        agent="target-model-calibration",
        label="target-model-calibration",
        strategy="Evaluate the candidate wrapper on benign OR-Bench prompts and judge whether it over-refuses.",
        rationale="A prompt fails when the assistant refuses a benign request that should have been answered.",
        candidate_summary="OR-Bench hard-1k over-refusal evaluation run.",
    ...
    return build_benchmark_adapter_result(
        ...
        baseline=baseline,
        winner=winner,
        selection_reason=f"Evaluated {total} OR-Bench hard prompts with over_refusal_rate={rate:.6f}.",
        extra_fields={
