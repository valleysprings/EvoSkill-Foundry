...
FULL_DATASET_SIZE = 1319


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Materialize the OR-Bench hard-1k manifest.")
...
def build_questions(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row_index, row in enumerate(rows, start=1):
        ...
        items.append(
            {
                "item_id": f"or-bench-hard-{row_index:04d}",
                "name": f"OR-Bench Hard #{row_index}",
                "prompt": prompt,
                "context": "",
...
def main() -> None:
    ...
    print(f"Wrote {len(prepared_items)} OR-Bench hard prompts to {MANIFEST_PATH}.")
