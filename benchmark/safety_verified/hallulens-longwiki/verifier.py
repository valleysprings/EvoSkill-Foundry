from __future__ import annotations

from pathlib import Path

from app.bench.safety_support import evaluate_single_turn_grounded_item_candidate


TASK_ROOT = Path(__file__).resolve().parent
BENCHMARK_NAME = "hallulens-longwiki"


def evaluate_candidate(*, task, candidate_path, source_code, baseline_metrics, memory_applied):
    del source_code, baseline_metrics, memory_applied, TASK_ROOT
    item = task.get("question_item")
    if not isinstance(item, dict):
        raise ValueError("Dataset question task must provide question_item.")
    return evaluate_single_turn_grounded_item_candidate(
        task=task,
        candidate_path=Path(candidate_path),
        item=item,
        benchmark_name=BENCHMARK_NAME,
    )
