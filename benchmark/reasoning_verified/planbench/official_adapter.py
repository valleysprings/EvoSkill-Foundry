...
from app.configs.paths import RUNTIME_BENCHMARKS_ROOT


PLANBENCH_RUNTIME_ROOT = RUNTIME_BENCHMARKS_ROOT / "planbench"
DEFAULT_OFFICIAL_ROOT = PLANBENCH_RUNTIME_ROOT / "plan-bench"
DEFAULT_VAL_BINARIES = (
    PLANBENCH_RUNTIME_ROOT / "VAL" / "build" / "bin" / "validate",
    PLANBENCH_RUNTIME_ROOT / "VAL" / "build" / "bin" / "Validate",
)
...
def resolve_official_root() -> Path:
    ...
    root = Path(explicit).expanduser() if explicit else DEFAULT_OFFICIAL_ROOT
    if (root / "configs").exists() or (root / "instances").exists():
        return root
    nested = root / "plan-bench"
    if (nested / "configs").exists() or (nested / "instances").exists():
        return nested
    return root
...
def _extract_obfuscated_plan(text: str, config: PlanBenchConfig) -> str:
    ...
    for line in [value.strip() for value in text.split("\n")]:
        ...
        object_ids = [
            token.strip()
            for token in line.split(object_prefix)
